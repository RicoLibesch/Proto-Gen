...
export const selectAllUsers = async (page: number, pageSize: number, userId: string, roleId: number): Promise<User[]> => {
    try {
        const users: User[] = [];
        ...
        if(roleId) {
            userData = await pool.query('SELECT * FROM users WHERE LOWER(id) LIKE $1 AND id IN (SELECT user_id FROM user_roles WHERE role_id = $2) ORDER BY id DESC OFFSET $3 LIMIT $4', [`%${userId}%`, roleId, offset, pageSize]);
        } else {
            userData = await pool.query('SELECT * FROM users WHERE LOWER(id) LIKE $1 ORDER BY id DESC OFFSET $2 LIMIT $3', [`%${userId}%`, offset, pageSize]);
        }
